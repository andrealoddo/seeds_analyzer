...
public class Seeds_Analysis implements PlugIn
{
	...
	* richiama il metodo catch_parasite_running dandogli in ingresso l'immagine */
	...
	{
		/* Alessandra's code integration */
		ImagePlus imp, imLight;
		ImagePlus imDark, imSubtracted = null;
		ImagePlus RGB;
		...
		if (op.contentEquals("Select WHITE and BLACK background images"))
		{
			...
			imSubtracted = ic.run("Subtract create", imDark, imLight);
			imSubtracted.show();
		...
		// Original run() method code

		// Aggiungere l'operazione di OR tra RGB sfondo chiaro e la mask risultante dalla conversione a maschera di imSubtracted
		// Lavoro solo sull'RGB_mask che mi sono prodotto!
		// Se ci sono problemi con la soglia a colori, impostare a 1 il minimo di ogni banda, tanto lo sfondo di RGB_mask è uniforme
		//imp = IJ.getImage();
		imp = imSubtracted;
